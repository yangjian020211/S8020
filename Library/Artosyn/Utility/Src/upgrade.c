...
#define UPGRADE_SESSION         BB_COM_SESSION_4
...
uint32_t UPGRADE_SendDataBlock(uint8_t u8_opt, uint8_t *pu8_dataBuff, uint16_t u16_dataLen, uint16_t u16_dataBlockNum)
{
    ...
    st_upgrade.par.upgradeDataPack.u16_data[2] = 0;
     u32_mask = local_irq_disable_save_flags();
    MD5SUM_Generate(pu8_dataBuff, u16_dataLen, st_upgrade.par.upgradeDataPack.u8_md5);
     local_irq_restore(u32_mask);

    // DLOG_Critical("-----num = %d  len = %d", st_upgrade.par.upgradeDataPack.u16_data[0],st_upgrade.par.upgradeDataPack.u16_data[1]);
...
static void UPGRADE_Update_StatetoApp(STRU_upgardeMsg *st,int result){
	STRU_SysEvent_UPGRADE_RecMSG msg ={0};
	msg.received_num=s_u16_dataBlockNum;
	msg.lost_num=s_u16_lostDataBlockCount;
	msg.result=result;
	msg.state = st->par.upgradeDataPack.u8_opt;
	msg.total_num=s_u16_upgradeReturnStatus;
...
static void UPGRADE_rcvInit_proc(STRU_upgardeMsg *st)
{
    ...
    (*(volatile uint32_t *)0xA0030078) = 0;//reset sdram
	...
	UPGRADE_Update_StatetoApp(st,0);
...
static void UPGRADE_rcvPacket_proc(STRU_upgardeMsg *st, uint8_t *u8_recDataArray, uint16_t u16_dataBlockNum, uint16_t u16_dataLen)
{
    ...
    {	       
		...
		UPGRADE_Update_StatetoApp(st,1);
    }
}
...
static void UPGRADE_rcvEnd_proc(STRU_upgardeMsg *st)
{
    ...
    if(s_u16_lostDataBlockCount == 0)	    //successful
    {
        ...
        if (-1 == UPGRADE_MD5SUM(s_u32_imageAddr, 1))
        {
			...
			UPGRADE_Update_StatetoApp(st,3);
            return;
        }
        else
        {
            ...
            {			    
                if (0 == UPGRADE_ModifyBootInfo(APPLICATION_IMAGE_START0))   
                {
					...
					UPGRADE_Update_StatetoApp(st,2);
                }
                else
                {
                    DLOG_Critical("upgrade fail");
                    st->par.upgradeDataPack.u8_opt = UPGRADE_ACK_FAIL;
					UPGRADE_Update_StatetoApp(st,3);
                }			 
            }
        ...
        while(1)
        {
            ...
            DLOG_Critical("send upgrade status1");
            while (0 != UPGRADE_SendMsg(st, NULL, 0))
            {
                HAL_Delay(UPGRADE_DELAY);
            }
            DLOG_Critical("send upgrade status2");
            HAL_Delay(1000);
            while (0 != UPGRADE_SendMsg(st, NULL, 0))
            {
                HAL_Delay(UPGRADE_DELAY);
            }
            DLOG_Critical("send upgrade status3");
            board_reset_all();
        }
    }
...
int8_t UPGRADE_WirelessGndSendInit(uint32_t u32_dataLen)
{
    ...
    DLOG_Warning("st_upgrade total packet = %d", u32_dataLen);
    return UpgradeGndStatus_Get(UPGRADE_ACK_INIT);
}
...
int8_t UPGRADE_UseUsbUpgrade(const uint8_t *fileName, uint8_t u8_flag)
{
    ...
    if (1 == u8_flag)
    {
        return UPGRADE_CheckUpgradeRemoteFile(path);
    }
    else
    {
        return UPGRADE_CheckUpgradeLocalFile(path);
    }
}
