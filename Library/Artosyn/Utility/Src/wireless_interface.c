...
uint8_t WIRELESS_INTERFACE_GET_VERSION_Handler(void *param, uint8_t id)
{
    ...
    {
    	// DLOG_Error("header");
    	//for(i=0;i<10;i++)DLOG_Critical("%2x",*((uint8_t *)msg_header+i));
		//DLOG_Error("WIRELESS_PROTOCAL_VERSION");

		//for(i=16;i<32;i++)DLOG_Critical("%d",*((uint8_t *)msg_header+i));
...
/*
static void UPGRADE_Update_StatetoApp(int result){
	STRU_SysEvent_UPGRADE_RecMSG msg ={0};
	msg.received_num=0;
	msg.lost_num=0;
	msg.result=result;
	msg.state = 0;
	msg.total_num=0;
	SYS_EVENT_NotifyInterCore(SYS_EVENT_ID_UPGRADE_STATE_EVENT, (void*)&msg);
}
*/
HAL_RET_T set_upgrade_flag(uint32_t en)
{
	uint8_t u8_ret;
	STRU_WIRELESS_CONFIG_CHANGE st_cmd;
	
	st_cmd.u8_configClass  = WIRELESS_RF_REMOTE_UPGRADE;
	st_cmd.u32_configValue = en;
    ...
    while( 0 == UpgradeGndStatus_Get(UPGRADE_ACK_CHECK) )
    {
	    ...
	    tick = SysTicks_GetUsTickCount();
		    ...
		    msgHeader->magic_header[0] = 0xFF;
                    ...
                    msgHeader->packet_num = 1;
                    msgHeader->chk_sum = 1;
                    memset(msgContent, 0, sizeof(u8_upgradeReply));
		    msgContent[0] = 1;
		    ...
		    tick_end = SysTicks_GetUsTickCount();
		    	...
		    	DLOG_Critical("sys_tick 2 %d, tick %d ", tick_end, tick);
		        break;
                    }
...
#ifdef C201S_UAV
        if (u8_upgradeUSBId == 0)
        {
            uint32_t reg_val = *(uint32_t *)(0x43100810);
            reg_val |= ((uint32_t)0x00000010);
            *(volatile uint32_t *)(0x43000810) = reg_val;
        }
        else
        {
            uint32_t reg_val = *(uint32_t *)(0x43000810);
            reg_val |= ((uint32_t)0x00000010);
            *(volatile uint32_t *)(0x43000810) = reg_val;
        }
#endif
            ...
            upgrade_type == 0x04 || upgrade_type == 0x05)
        {
            ...
            if(id == 0)
            {
              /*
                ...
                USBx_DEVICE->DIEPMSK = 0x0000000b;
              **/
 
                uint32_t reg_val = *(uint32_t *)(0x43100810);
                reg_val &= ~((uint32_t)0x00000010);        //((uint32_t)0x00000010) USB_OTG_DIEPMSK_ITTXFEMSK
                *(volatile uint32_t *)(0x43100810) = reg_val;
            }
            else
            {
                uint32_t reg_val = *(uint32_t *)(0x43000810);
...
uint8_t WIRELESS_INTERFACE_GET_DEV_INFO_Handler(void *param, uint8_t id)
{
	...
	DLOG_Critical("return size %d", pstDeviceInfo->msg_len);
	if (HAL_USB_Get_USB_DR_Mode(id) == HAL_USB_DR_MODE_HOST)
	{
		if(g_mtp_enable)
...
uint8_t WIRELESS_INTERFACE_SELECT_VIDEO_FREQ_CHANNEL_Handler(void *param, uint8_t id)
{
    ...
    if (WIRELESS_IsInDebugMode() == 1)
    {
        ...
        {
            //BB_SPI_curPageWriteByte(0x01,0x01);
            if(msgContent[6] >= 0x5a && msgContent[6] < 0x79)//8003s, 5.8G
            {
                for (u8_index = 0; u8_index < msgContent[2]; ++u8_index)
                {
                    RF_SPI_WriteReg(0x6f-u8_index,msgContent[u8_index+3]);//88 88 88 5c
                    DLOG_Warning("5g:addr=%02x,value=%02x",0x6f-u8_index,msgContent[u8_index+3]);
                }

                RF_SPI_WriteReg(0x69, 0x01);
            }
            else//8003s, 2.4G and 8003
            {
                for (u8_index = 0; u8_index < msgContent[2]; ++u8_index)
                {
                    RF_SPI_WriteReg(0x20-u8_index,msgContent[u8_index+3]);//88 88 88 5c
                    DLOG_Warning("2g:addr=%02x,value=%02x",0x20-u8_index,msgContent[u8_index+3]);
                }

                RF_SPI_WriteReg(0x69, 0x00);
            }
            //BB_SPI_curPageWriteByte(0x01,0x02);
            return 0;
        }
        ...
        for (u8_index = 0; u8_index < msgContent[2]; ++u8_index)
        {
            BB_SPI_WriteByte(msgContent[0],msgContent[1],msgContent[u8_index+3]);//88 88 88 5c
            msgContent[1] += 1;
        }

        //BB_SPI_curPageWriteByte(0x00,0x01);
        //BB_SPI_curPageWriteByte(0x00,0x00);
...
uint8_t WIRELESS_INTERFACE_WRITE_RF9363_Handler(void *param, uint8_t id)
{
    ...
    u16_addr = ((msgContent[0]) << 8) | msgContent[1];

    if (WIRELESS_IsInDebugMode() == 1)
    {
        if (RF_SPI_WriteReg(u16_addr, msgContent[2]))
        {
            DLOG_Error("write fail!\n");
            return 1;
        }
...
uint8_t WIRELESS_INTERFACE_WRITE_RF_REG_Handler(void *param, uint8_t id)
{
    ...
    if (WIRELESS_IsInDebugMode() == 1)
    {
        if (RF_SPI_WriteReg(msgContent[0], msgContent[1]))
        {
            DLOG_Error("write fail!\n");
...
uint8_t WIRELESS_INTERFACE_READ_RF9363_Handler(void *param, uint8_t id)
{
    ...
    addr = (msgContent[0] << 8) | msgContent[1];

    if (WIRELESS_IsInDebugMode() == 1)
    {
        RF_SPI_ReadReg(addr, &msgContent[2]);
        Wireless_InsertMsgIntoReplyBuff(0x4A, 3, 1, 0, msgContent, id);
    }
}
...
uint8_t WIRELESS_INTERFACE_READ_RF_REG_Handler(void *param, uint8_t id)
{
    ...
    if (WIRELESS_IsInDebugMode() == 1)
    {
        RF_SPI_ReadReg(msgContent[0], &msgContent[1]);

        Wireless_InsertMsgIntoReplyBuff(0x3A, 2, 1, 0, msgContent, id);
    }
    else
    {
        Wireless_InsertMsgIntoReplyBuff(0x0f, 0, 1, 0, NULL, id);
