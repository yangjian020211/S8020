...
{
  BF_PartHolder *newPH = calloc_safe(1, sizeof(BF_PartHolder));
  /* assert( newPH ); */
  newPH->max_elements = max_elements;
  newPH->part = calloc_safe(1, sizeof(BF_BitstreamPart));
  /* assert( newPH->part ); */
  newPH->part->element = calloc_safe((int)max_elements, sizeof(BF_BitstreamElement));
...
BF_PartHolder *shine_BF_freePartHolder( BF_PartHolder *thePH )
{
  ...
  free_safe( thePH->part->element );
  free_safe( thePH->part );
  free_safe( thePH );
  return NULL;
}
