...
//#include "ss_hal_core.h"
#include "ss_test_datalog.h"
#include "ss_test_data_persist.h"

typedef void (*ss_test_func)(void);
static ss_test_func init_test_list[] = {
    ss_test_datalog_init,
    ss_test_data_persist_init,
};
static ss_test_func run_test_list[] = {
     ss_test_datalog_update,
     ss_test_data_persist_update,
};
...
void test_hal_sys_task(void*){
    ...
    static uint16_t init_test_list_count = sizeof(init_test_list) / sizeof(ss_test_func);
    static uint16_t run_test_list_count = sizeof(run_test_list) / sizeof(ss_test_func);
    ...
    for (int i = 0; i < init_test_list_count; ++i) {
        init_test_list[i]();
    }
    ss_hal_debuglog_set_block(false);
    while (1) {
        fixed_fps1();
        for (int i = 0; i < run_test_list_count; ++i) {
            run_test_list[i]();
        }
        ss_hal_idle(500);
    }
