...
static void COMTASK_FlushTxBufferToUart(osMessageQId QueueId)
{
    if (QueueId != 0)
    {
        if (QueueId == com_queue0_id)
        {
            ...
            if (tx_buf0_index != 0)
            {
                HAL_BB_ComSendMsg(BB_COM_SESSION_1, tx_buf0, tx_buf0_index);
                tx_buf0_index = 0;
            }
        ...
        else if (QueueId == com_queue1_id)
        {
            ...
            if (tx_buf1_index != 0)
            {
                HAL_BB_ComSendMsg(BB_COM_SESSION_2, tx_buf1, tx_buf1_index);
                tx_buf1_index = 0;
            }
...
static void COMTASK_Tx0Function(void const *argument)
{
    ...
    HAL_BB_ComRegisterSession(BB_COM_SESSION_1,
...
static void COMTASK_Rx0Function(void *p)
{
    uint32_t cnt = 0;

    HAL_BB_ComReceiveMsg(BB_COM_SESSION_1, rx_buf1, sizeof(rx_buf1), &cnt);

    HAL_UART_TxData(HAL_UART_COMPONENT_3, rx_buf1, cnt, HAL_UART_DEFAULT_TIMEOUTMS);
}

static void COMTASK_Rx1Function(void *p)
{
    uint32_t cnt = 0;

    HAL_BB_ComReceiveMsg(BB_COM_SESSION_2, rx_buf0, sizeof(rx_buf0), &cnt);

    HAL_UART_TxData(HAL_UART_COMPONENT_4, rx_buf0, cnt, HAL_UART_DEFAULT_TIMEOUTMS);
}
