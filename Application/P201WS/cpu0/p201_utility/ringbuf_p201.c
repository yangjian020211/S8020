...
int
ringbuf_memcpy_into_1(ringbuf_t dst, const void *src, size_t count)
{
    const uint8_t *u8src = src;
    const uint8_t *bufend = ringbuf_end(dst);
    int overflow = count > ringbuf_bytes_free(dst);
...
uint8_t
ringbuf_watch(ringbuf_t src, size_t pos)
{
    const uint8_t *bufend = ringbuf_end(src);
	if(src->tail + pos >= bufend){
		return *(src->buf + (src->tail + pos - bufend));
	}
	else{
		return *(src->tail + pos);
	}
...
void *
ringbuf_move(ringbuf_t src, size_t count)
{
    size_t bytes_used = ringbuf_bytes_used(src);
    if (count > bytes_used)
        return 0;
