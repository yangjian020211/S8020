...
#define SKY_DELAY_HAL_TIMER        	(HAL_TIMER_NUM13)
#define SKY_DELAY_HAL_TIMER_INTR	(TIMER_INTR13_VECTOR_NUM)

#define SKY_TX_ENABLE_OPEN_TIMER        	(HAL_TIMER_NUM12)
#define SKY_TX_ENABLE_OPEN_HAL_TIMER_INTR	(TIMER_INTR12_VECTOR_NUM)


/* define rt uart read BaseBasd time in sky */
#define SBUS_SKY_POLLING_DATA_TIME_WINDOW    (5*1000)

#define SKY_TX_SYNC_GPIO          (HAL_GPIO_NUM87)
#define BT_TX_EN_GPIO          (HAL_GPIO_NUM69)
#define SKY_TX_DATA_TIME_WINDOW    (8*1000)
#define SKY_TX_ENABLE_OPEN_WINDOW    (12*1000-SKY_TX_DATA_TIME_WINDOW)
...
static void bypass_bb_spi_irq_handler(void *p)
{
    ...
    //if(!pst_spiDSave->rsv && cnt > 0)// bb not locked, drop data
    //{
    //    DLOG_Warning("not lock, drop data %d",cnt);
    //    return;
    //}
...
static void hal_uart_tx2bb_spi(void)
{
    ...
    #if 0 //debug simulation rt-uart send
    rt_uart_cnt = 10;
    #endif
    ...
    if(rt_uart_cnt != spi_num && print_cnt++ > 200 )
    {
        print_cnt = 0;
    ...
    ret = HAL_BB_ComSendMsg(BYPASS_BB_SESSION,rt_uart_buf,len);
    if(ret != HAL_OK)
    {
...
{     
    DLOG_Info("------> hal_uart_rx = %d, bb_uart_rx= %d, write bb = %d",
...
static void sky_tx_enable_open(uint32_t value)
{
	HAL_TIMER_Stop(SKY_TX_ENABLE_OPEN_TIMER);
	NVIC_ClearPendingIRQ(BB_TX_ENABLE_VECTOR_NUM);
    NVIC_EnableIRQ(BB_TX_ENABLE_VECTOR_NUM);
}


void sky_tx_delay_notice(uint32_t value)
{
    HAL_RET_T ret;
	HAL_GPIO_SetPin(SKY_TX_SYNC_GPIO,HAL_GPIO_PIN_RESET);
    ret = HAL_TIMER_Stop(SKY_DELAY_HAL_TIMER);
    if(ret != HAL_OK)
    {
        DLOG_Error("stop timer failed");
    }
	 HAL_TIMER_Start(SKY_TX_ENABLE_OPEN_TIMER);
   
}

static void sky_bb_tx_irq(uint32_t value)
{
    HAL_RET_T ret;
    
    HAL_GPIO_SetPin(SKY_TX_SYNC_GPIO,HAL_GPIO_PIN_SET);
    NVIC_DisableIRQ(BB_TX_ENABLE_VECTOR_NUM);
    ret = HAL_TIMER_Start(SKY_DELAY_HAL_TIMER);
    if(ret != HAL_OK)
    {
        DLOG_Error("start timer failed");
    }
...
void sky_bb_tx_init(void)
{
    //init gpio to output and default level is low
    HAL_GPIO_SetMode(SKY_TX_SYNC_GPIO, HAL_GPIO_PIN_MODE2);
    HAL_GPIO_OutPut(SKY_TX_SYNC_GPIO);
    HAL_GPIO_SetPin(SKY_TX_SYNC_GPIO,HAL_GPIO_PIN_RESET);

    HAL_GPIO_SetMode(BT_TX_EN_GPIO, HAL_GPIO_PIN_MODE2);
    HAL_GPIO_OutPut(BT_TX_EN_GPIO);
    HAL_GPIO_SetPin(BT_TX_EN_GPIO,HAL_GPIO_PIN_RESET);	
    //register BaseBand Irq, when BaseBand Tx,will generate TX_ENABLE irq
    reg_IrqHandle(BB_TX_ENABLE_VECTOR_NUM,sky_bb_tx_irq,NULL);
    NVIC_SetPriority(BB_TX_ENABLE_VECTOR_NUM,NVIC_EncodePriority(NVIC_PRIORITYGROUP_5,INTR_NVIC_PRIORITY_UART_DEFAULT,0)); 
    //register timer to 
    if (HAL_OK != HAL_TIMER_RegisterTimer(SKY_DELAY_HAL_TIMER, SKY_TX_DATA_TIME_WINDOW, sky_tx_delay_notice))
    {
        DLOG_Critical("--->failed register timer");
        return;
    }
	 NVIC_SetPriority(SKY_DELAY_HAL_TIMER_INTR,NVIC_EncodePriority(NVIC_PRIORITYGROUP_5,INTR_NVIC_PRIORITY_UART_DEFAULT,0));
	if (HAL_OK != HAL_TIMER_RegisterTimer(SKY_TX_ENABLE_OPEN_TIMER, SKY_TX_ENABLE_OPEN_WINDOW, sky_tx_enable_open))
    {
        DLOG_Critical("--->failed register timer");
        return;
    }
    NVIC_SetPriority(SKY_TX_ENABLE_OPEN_HAL_TIMER_INTR,NVIC_EncodePriority(NVIC_PRIORITYGROUP_5,INTR_NVIC_PRIORITY_UART_DEFAULT,0));
