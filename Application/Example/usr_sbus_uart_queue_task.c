...
#define RT_UART_BUF_Q_LEN 3
static int hal_uart_rx_cnt=0,bb_uart_rx_cnt=0,write_bb_cnt=0;//debug info 
static uint8_t spi_num;
static uint8_t rt_uart_buf[RT_UART_BUF_Q_LEN][MAX_RT_BUF_LEN];
static uint8_t rt_uart_buf_len[RT_UART_BUF_Q_LEN];
static uint8_t rt_uart_buf_index_w = 0,rt_uart_buf_index_r = 0;
//static volatile uint8_t rt_uart_cnt=0;
uint32_t rx_packet_start_time;
/**
...
static uint32_t bypass_hal_uart_irq_handler(uint8_t *pu8_rxBuf, uint8_t u8_len)
{
	...
	if(rt_uart_buf_len[rt_uart_buf_index_w] == 0){
		rx_packet_start_time = SysTicks_GetTickCount();
	}
	else{
		if(SysTicks_GetDiff(rx_packet_start_time, SysTicks_GetTickCount())  > 6){
			rt_uart_buf_index_w++;
			if(rt_uart_buf_index_w >= RT_UART_BUF_Q_LEN){
				rt_uart_buf_index_w = 0;
			}
			rt_uart_buf_len[rt_uart_buf_index_w] = 0;
			rx_packet_start_time = SysTicks_GetTickCount();
		}
	}
    i = u8_len + rt_uart_buf_len[rt_uart_buf_index_w];

    if(i > MAX_RT_BUF_LEN)
    {
        DLOG_Error("%d over max spi num",i);
        //rt_uart_buf_len[rt_uart_buf_index_w] += u8_len;
        return 0;
    }

    for(i=0;i<u8_len;i++)
    {
        rt_uart_buf[rt_uart_buf_index_w][rt_uart_buf_len[rt_uart_buf_index_w]+i] = pu8_rxBuf[i];
    }
	
    rt_uart_buf_len[rt_uart_buf_index_w] += u8_len;
	
	/*if(rt_uart_buf_len[rt_uart_buf_index_w] >= spi_num){
		rt_uart_buf_index_w++;
		if(rt_uart_buf_index_w >= RT_UART_BUF_Q_LEN){
			rt_uart_buf_index_w = 0;
		}
...
static void hal_uart_tx2bb_spi(void)
{
    ...
    if(rt_uart_buf_index_r == rt_uart_buf_index_w)
    {
        DLOG_Info("rt uart data read null");
        return;
    }
    
    if(rt_uart_buf_len[rt_uart_buf_index_r] != spi_num && print_cnt++ > 200 )
    {
        print_cnt = 0;
        DLOG_Warning("rt uart data len %d != %d",rt_uart_buf_len[rt_uart_buf_index_r],spi_num);
    }
    ...
    write_bb_cnt += len;
	rt_uart_buf_len[rt_uart_buf_index_r] = 0;
	rt_uart_buf_index_r++;
	if(rt_uart_buf_index_r >= RT_UART_BUF_Q_LEN){
		rt_uart_buf_index_r = 0;
	}
}
/**
