...
//    {5, bringup_usb0_test,      "usb0 test",    0,  0},
    ...
    {5, bringup_usb1_test,      "usb1 test",    0,  0},
    {6, bringup_adc_test,       "adc_test",     0,  0},
    {7, bringup_i2c_test,       "i2c test",     0,  0},
    {8, bringup_test_finish,    "test_finish",  0,  0},   
    {0, 0,                        "finished",     0,  0}
...
int bringup_usb1_test(char num)
{
    ...
    fileResult = f_read((&usbhAppFile), testbuf, 32, (void *)&bytesread);
...
*/
    ...
    for(i = 0; test_info[i].testnum; i++)
        if(test_info[i].result < 0)
        {
            DLOG_Critical("###Fail item: %s###",test_info[i].name);
            Autotest_status |= 1 << i;
            set_led(BLUE,OFF);
            set_led(RED,ON);
        }
