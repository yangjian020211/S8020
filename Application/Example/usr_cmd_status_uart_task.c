...
#include "debuglog.h"
#include "usr_cmd_status_uart_task.h"

#define CMD_STATUS_UART_NUM HAL_UART_COMPONENT_6
static osMutexId cmd_uart_mutex_id;
static uint8_t dev_type;

uint16_t  cmd_status_get_fac_update_time(uint8_t *enable)
{
...
ENUM_HAL_UART_BAUDR cmd_status_get_fac_uart6_baudrate(void)
{
...
static uint8_t get_ground_mod_status_info(uint8_t *buf)
{
    int len;
    uint16_t ret;
...
static uint8_t get_sky_mod_status_info(uint8_t *buf)
{
    int len;
    uint16_t ret;
...
static void cmd_execute(uint16 msg_id,uint8_t *pu8_rxBuf, uint16_t len)
{
    ...
    switch(msg_id)
    {
        ...
        case 0x0082:
            ...
            HAL_UART_TxData(CMD_STATUS_UART_NUM,buf,msg_len,1000);
...
static void cmd_mod_status_update_task(void const *argument)
{
    ...
    uint8_t buf[64],len;
	int i=0;
    update_time = cmd_status_get_fac_update_time(&enable);
	//enable = 1;
    ...
    while (1)
    {
        ...
        osMutexWait(cmd_uart_mutex_id,0);
        HAL_UART_TxData(CMD_STATUS_UART_NUM,buf,len,1000);
        osMutexRelease(cmd_uart_mutex_id);
		HAL_Delay(1000);
    }
}
...
void usr_cmd_status_uart_task(uint8_t dev_type_class)
{
    HAL_RET_T ret;
    
    ENUM_HAL_UART_BAUDR enum_baud = cmd_status_get_fac_uart6_baudrate();
    dev_type = dev_type_class;

    ret = HAL_UART_Init(CMD_STATUS_UART_NUM,enum_baud,hal_cmd_uart_irq_handler);
    if(ret != HAL_OK)
    {
        DLOG_Error("uart6 init failed");
        return;
    }
