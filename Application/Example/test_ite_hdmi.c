...
{
    uint32_t _bank = strtoul(bank, NULL, 0);
    uint32_t _rw = strtoul(rw, NULL, 0);
    uint32_t _reg = strtoul(reg, NULL, 0);
    uint32_t _value = strtoul(value, NULL, 0);

    if(rw)
    {
        chgbank(_bank);
        DLOG_Warning("page%d",_bank);
        DLOG_Warning("reg %02x=%02x\n",_reg,hdmirxrd((uint8_t)_reg));
    }
    else
    {
        chgbank(_bank);
        DLOG_Warning("page%d",_bank);
        hdmirxwr((uint8_t)_reg,(uint8_t)_value);
        DLOG_Warning("reg %02x=%02x\n",_reg,hdmirxrd((uint8_t)_reg));
