...
uint32_t bypass_write_usb2uart(uint8_t *pu8_rxBuf, uint16_t u16_len)
{
    ...
    if(bb_dev_type == 0 && get_link_status() != LINK_LOCK)//only sky care lock status
    {
        return 0;
    }
...
static void bypass_bb_uart_irq_handler(void *p)
{
    ...
    {
        DLOG_Warning("uart not lock, drop %d",cnt);
        return;
    }
...
static void bb_uart_tx2hal_task(void const *argument)
{
    ...
    while (1)
    {
        ...
        ret = HAL_UART_TxData(uart_bypass_num, buffer, len, HAL_UART_DEFAULT_TIMEOUTMS * 1000);
        if(ret != HAL_OK)
        {
            DLOG_Error("failed ret %d",ret);
        }
...
static void bypass_uart_log_task(void const *argument)
{
    while (1)
    {
        DLOG_Info("------> hal_uart_rx = %d",hal_uart_rx_cnt);
        DLOG_Info("------> bb_uart_rx = %d",bb_uart_rx_cnt);
        HAL_Delay(5000);
    }
}
...
{
    bb_dev_type = dev_type;
    uart_bypass_num = uart_num;
    bypass_mode = get_fac_bypass_mode();
    ENUM_HAL_UART_BAUDR enum_baud =  get_fac_uart5_baudrate();
    DLOG_Warning("bypass uart num %d, br=%d",uart_bypass_num,enum_baud);
    ...
    if(dev_type)
    {
        if(bypass_mode == 1 || bypass_mode == 2 || bypass_mode == 4 || bypass_mode == 6)
        {
            HAL_UART_Init(uart_bypass_num,enum_baud,bypass_hal_uart_irq_handler);
        }
        else
        {
            HAL_UART_Init(uart_bypass_num,enum_baud,nothing_todo);
        }
    }
    else
    {
        if(bypass_mode == 1 || bypass_mode == 3 || bypass_mode == 5 || bypass_mode == 6)
        {
            HAL_UART_Init(uart_bypass_num,enum_baud,bypass_hal_uart_irq_handler);
        }
        else
