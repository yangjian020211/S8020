...
{
    if(bb_dev_type == 0 && get_link_status() != LINK_LOCK)
    {
        return 0;
    }
...
static void BB_Com_Rcv_Data_Handler(void *p)
{
    ...
    if(bb_dev_type == 0 && get_link_status() != LINK_LOCK && cnt > 0)
    {
...
STRU_WIRELESS_USER_HANDLER   Cmd_Handler[CMD_HANDLER_CNT] = {
	...
	{CMD_MANUL_START_LOG,cmd_ack_handle},
	{CMD_MANUL_STOP_LOG,cmd_ack_handle},
	{CMD_SET_RF_BAND,cmd_ack_handle},
};
...
static uint32_t bypass_hal_uart4_irq_handler(uint8_t *pu8_rxBuf, uint8_t u8_len)
{
    usb_bypass_write(pu8_rxBuf,u8_len);
}

static void bb_uart4_tx2hal_task(void const *argument)
{
    int ret;
    ...
    while(1)
    {
        ...
        if(ret > 0)
        {
            ...
            if(HAL_OK != HAL_UART_TxData(BYPASS_USB2UARTx_NUM,bypass_to_host,ret,HAL_UART_DEFAULT_TIMEOUTMS * 1000))
            {

                DLOG_Critical("uart6 send err");
...
void usr_usb0_interface(void)
{
    ...
    bb_dev_type = pst_cfg->u8_workMode;
    ...
    if(bypass_mode == 6 && pst_cfg->u8_workMode == 0)
    {
        HAL_UART_Init(BYPASS_USB2UARTx_NUM,enum_baud,bypass_hal_uart4_irq_handler);
		
		osThreadDef(COMTASK_Tx1, bb_uart4_tx2hal_task, osPriorityHigh, 0, 4 * 128);
