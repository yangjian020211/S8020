...
void SetCameraPower(void)
{
    HAL_GPIO_SetMode(HAL_GPIO_NUM89, HAL_GPIO_PIN_MODE2);
    HAL_GPIO_OutPut(HAL_GPIO_NUM89);
    HAL_GPIO_SetPin(HAL_GPIO_NUM89, HAL_GPIO_PIN_SET);	
    
    HAL_GPIO_SetMode(HAL_GPIO_NUM51, HAL_GPIO_PIN_MODE2);
    HAL_GPIO_OutPut(HAL_GPIO_NUM51);
...
int main(void)
{
    ...
    osThreadDef(Camera, Camera_Task, osPriorityNormal, 0, 16 * 128);
    osThreadCreate(osThread(Camera), NULL);
