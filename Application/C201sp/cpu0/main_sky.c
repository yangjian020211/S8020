...
int prepercent=-1;
void Sky_UPGRADE_EventHandler(void *p){

	STRU_SysEvent_UPGRADE_RecMSG* msg = (STRU_SysEvent_UPGRADE_RecMSG*) p;
	int percent = (msg->received_num-msg->lost_num)*100/msg->total_num;
	
	if(prepercent !=percent){
		DLOG_Critical("total_num=%d,state=%d,received_num=%d,lost_num=%d,result=%d,percent=%d%%",
		msg->total_num,msg->state,msg->received_num,msg->lost_num,msg->result,percent);
		prepercent = percent;
	}
}
...
int main(void)
{
	...
	SYS_EVENT_RegisterHandler(SYS_EVENT_ID_UPGRADE_STATE_EVENT, (SYS_Event_Handler)Sky_UPGRADE_EventHandler);
