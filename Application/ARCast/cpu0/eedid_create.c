...
const uint8_t detail_timing[9][18] = {
    ...
    {0x01, 0x1d, 0x00, 0xbc, 0x52, 0xd0, 0x1e, 0x20, 0xb8, 0x28, 0x55, 0x00, 0xfd, 0x1e, 0x11, 0x00, 0x00, 0x18},//1280x720p@50
...
static int8_t create_edid1(struct edid * edid, const STRU_ARCAST_AVCAPABILITY *buff)
{
    ...
    DLOG_Error("preferred %d",buff->u8_VideoVidList[0]);
    ...
    for (uint8_t i = 0; i < len; i++)
    {
        switch(buff[i])
        {
            case 1:
                ...
                DLOG_Error("audio 44.1K");
                break;
            case 2:
                sad->sample_rate_48_kHz = 0x01;
                DLOG_Error("audio 48K");
                break;
            default:
                break;        
...
static int8_t create_edid_extension(struct edid_extension * ext, const STRU_ARCAST_AVCAPABILITY *buff)
{
    ...
    DLOG_Error("modify detail timing\n");
