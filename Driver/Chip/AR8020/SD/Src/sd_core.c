...
{
  volatile uint32_t get_val, cmd_done;
...
{
  volatile uint32_t get_val, data_over;
  uint32_t start;
  start = SysTicks_GetTickCount();
...
{
  volatile uint32_t get_val, card_busy;
  uint32_t start;
  start = SysTicks_GetTickCount();
...
{
  volatile uint32_t get_val, cmd_start;
  uint32_t start;
  start = SysTicks_GetTickCount();
...
{
  volatile uint32_t get_val, volt_switch_int;
  uint32_t start;
