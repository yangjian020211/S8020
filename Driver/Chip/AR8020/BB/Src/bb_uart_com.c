...
void session_cnt_debug_print(void)
{
    ...
    for(i=0;i<BB_COM_SESSION_SPI;i++)
        DLOG_Error("s%d ecnt %d tx %d rx %d",i,session_event_cnt[i],session_cnt_tx[i],session_cnt_rx[i]);
}
                       ...
                       uint16_t length)
{
    ...
    //DLOG_Error("%d %d",length,SysTicks_GetTickCount());
    //BB_ComLockRelease();
...
uint32_t BB_ComGetTXQueueFreeLength(STRU_BBComTxQueue *pst_txQueue)
{
	...
	DLOG_Info("max_size = %d,wr_pos=%d,rd_pos=%d,free_length=%d",max_size,wr_pos,rd_pos,free_length);
    return free_length;
}
                        ...
                        uint8_t *spi_info)
{
    ...
    if (BB_COM_TYPE_UART == e_bbComType) // uart channel
    {
        ...
        if (uart_send_size != 0)
        {
            uart_putdata(BBCOM_UART_INDEX,  uart_send_buff, uart_send_size);
			if(uart_send_size >0)
            DLOG_Info("1 %d %d %d",uart_max_tx_size,uart_send_size,SysTicks_GetTickCount());
        }
    ...
    else // spi channel
    {
		...
		if(spi_send_size >0)
        DLOG_Info("2 %d %d %d",spi_max_tx_size,spi_send_size,SysTicks_GetTickCount());
    }
