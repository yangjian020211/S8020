...
typedef struct
{
    ...
    uint8_t  flag_snrJudgebandchange;
...
void grd_get_snr(void)
{
    ...
    work_ch_snr.snr[work_ch_snr.row_index] = stru_grdstatus.u16_grd_snr;
...
void grd_set_txmsg_mcs_change(ENUM_CH_BW bw, uint8_t index)
{
    ...
    BB_WriteRegMask(PAGE2, MCS_INDEX_MODE, index,0x0f);
...
#define MCS2_UP_ERR_THLD       (540)    //10%  (QPSK, 1/2)   64 * 200 * 10% = 1280;
#define MCS3_UP_ERR_THLD       (10)    //5%   (16QAM, 1/2)  128* 200 * 5%  = 1280
#define MCS4_UP_ERR_THLD       (10)       //2.5% (64QAM, 1/2)  192* 200 * 2.5%= 960
...
#define MCS3_DOWN_ERR_THLD       (200)     //40%  (16QAM, 1/2) -> (QPSK, 1/2):    128 * 100 * 40% = 5120
#define MCS4_DOWN_ERR_THLD       (200)     //30%  (64QAM, 1/2) -> (16QAM, 1/2)    192 * 100 * 30% = 5760
#define MCS5_DOWN_ERR_THLD       (150)     //25%  (64QAM, 2/3) -> (64QAM, 1/2)    192 * 100 * 25% = 4800
...
static void grd_start_SnrHarqCnt(void)
{
    work_ch_snr.u8_isFull = 0;
    work_ch_snr.row_index = 0;
    work_ch_snr.flag_snrJudgebandchange = 0;
...
/*
 * QAM change:  depend on the SNR & LDPC error
 * band switch: if stay in MCS <= 1 more than 
 */
void grd_judge_qam_band_switch(void)
{
    ...
    if ( (!work_ch_snr.u8_isFull) || (!stru_ldpcErr.u8_isFull))
    {
        return;
    }
    ...
    if (flag_changemcs == 1)
    {
    	if(context.qam_ldpc < context.u8_bbStartMcs) 
			context.qam_ldpc = context.u8_bbStartMcs;
        grd_set_txmsg_mcs_change(context.st_bandMcsOpt.e_bandwidth, context.qam_ldpc);
        grd_start_SnrHarqCnt();
    }

    //if snr < snr_bpsk down snr
    work_ch_snr.flag_snrJudgebandchange = (get_snr_average() < qam_change_threshold[1][1]);
}


/*
 * check if snr meet condition for BandChange
*/
uint8_t grd_snr_forBandChange(void)
{
    return work_ch_snr.flag_snrJudgebandchange;
}
...
{	
	...
	uint32_t snr_sum = 0;
	int i=0;
	//DLOG_Critical("---------------------------------begin init mcs --------------------------------------------------------");
	snr = grd_get_it_snr();
	//DLOG_Info("snr = %d",snr);
	if(snr <=qam_change_threshold[1][1])context.qam_ldpc = 1;
	else if(snr <=qam_change_threshold[2][1])context.qam_ldpc = 1;
	else if(snr <=qam_change_threshold[3][1])context.qam_ldpc = 2;	
	else if(snr <=qam_change_threshold[4][1])context.qam_ldpc = 3;	
	else if(snr <=qam_change_threshold[5][1])context.qam_ldpc = 4;	
	else context.qam_ldpc = 0;	
	//DLOG_Critical(" qam_ldpc = %d", context.qam_ldpc);
	grd_set_txmsg_mcs_change(context.st_bandMcsOpt.e_bandwidth, context.qam_ldpc);
}
