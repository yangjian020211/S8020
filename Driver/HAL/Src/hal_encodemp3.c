...
ENUM_HAL_SRAM_VIDEO_CHANNEL e_bypassVideoCh_mp3;
...
{          
    ...
    e_bypassVideoCh_mp3 = dataPath;
    HAL_SRAM_EnableSkyBypassVideo(e_bypassVideoCh_mp3); 
#if 0
    if (dataPath == 0)
    {
    ...
    if (0 != (*pu32_newPcmDataFlagAddr))
    {
...
#if 0
        HAL_SRAM_TransferBypassVideoStream(e_bypassVideoCh_mp3, pu16_rawDataAddr, (ADUIO_DATA_BUFF_LENGHT*sizeof(uint16_t)));
#else
        
        while (u32_tmpRawDataLenght) {
            //DLOG_Critical("encode mp3 ok %d\n", (*pu32_newPcmDataFlagAddr));
            ...
            HAL_SRAM_TransferBypassVideoStream(e_bypassVideoCh_mp3, pu8_data, s32_encodeLenght);
            i+=s32_encodeLenght;
        }
#if 0        
        ...
        for (j = 0; j < i; j++)
        {
            ...
            if(hal_mp3_check_header(header) < 0) {
                ...
                //DLOG_Critical("extra_bytes %d %x",extra_bytes, header);
                break;
            }
        }
        
        HAL_SRAM_TransferBypassVideoStream(e_bypassVideoCh_mp3, ch, extra_bytes+1);
        HAL_SRAM_TransferBypassVideoStream(e_bypassVideoCh_mp3, g_u8_userDataArray, 8);
        HAL_SRAM_TransferBypassVideoStream(e_bypassVideoCh_mp3,  &ch[extra_bytes], i-extra_bytes-1);
#endif
        //HAL_SRAM_TransferBypassVideoStream(e_bypassVideoCh_mp3, ch, i);
#endif        
        (*pu32_newPcmDataFlagAddr) = 0;
#if 0        


/*        uint64_t end64 = SysTicks_GetUsTickCount();
        DLOG_Critical("encode mp3 ok %d\n", (uint32_t)SysTicks_GetUsDiff(start64, end64));
        start64 = end64;*/
